...
import { exportAssistantAuditCleanupReviewCoverageReport } from "@/use-cases/assistant-saas-mode-service";

export async function GET(request: Request) {
  try {
    const user = await requireRole("admin");
    const { searchParams } = new URL(request.url);
    const data = await exportAssistantAuditCleanupReviewCoverageReport(
      {
