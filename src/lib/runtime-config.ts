...
const defaultLocalDataRoot = process.env.LOCAL_DATA_ROOT || (process.platform === "win32" ? "D:/architect-start-data" : join(tmpdir(), "architect-start-data"));

export const localDataRoot = normalizeFsPath(defaultLocalDataRoot);
...
export const localHolidayCacheRoot = join(localDataRoot, "cache", "holidays");
...
export const holidayApiServiceKey = process.env.HOLIDAY_API_SERVICE_KEY?.trim() || "";
