-- Expanded RBAC RLS and Storage policy boundary for Architect Start Preview.
-- Apply only to the preview Supabase project after a backup and explicit user approval.
...
create or replace function app_private.can_manage_project_membership_role(target_project_id uuid, target_role "ProjectMembershipRole")
returns boolean
language sql
stable
security definer
...
as $$
  select target_project_id is not null
    and (
      ...
      or (
        app_private.is_project_manager(target_project_id)
        and target_role in ('viewer'::"ProjectMembershipRole", 'editor'::"ProjectMembershipRole", 'member'::"ProjectMembershipRole")
      )
    );
$$;
...
for insert
to authenticated
with check (app_private.can_manage_project_membership_role(project_id, role));
...
for update
to authenticated
using (app_private.can_manage_project_membership_role(project_id, role))
with check (app_private.can_manage_project_membership_role(project_id, role));
...
with check (
  bucket_id = 'task-files'
  and app_private.can_write_task(
...
using (
  bucket_id = 'task-files'
  and app_private.can_write_project(app_private.storage_object_project_id(name))
)
with check (
  bucket_id = 'task-files'
  and app_private.can_write_task(
