...
- 이 문서는 "지금 워크트리 상태 전체를 기준점으로 고정한다"는 전제로 작성한다.
- 즉, 현재 미커밋 변경도 기준점에 포함한다.
- 이후 스프레드시트 성능 전환 작업은 `phase branch -> integration branch -> 원래 브랜치 통합` 순서로 진행한다.
- 이 문서는 문서화만 수행한 상태이며, 아래 git 명령은 아직 실행되지 않았다고 가정한다.

## 2. 현재 시점 기준 스냅샷

작성 시점에 확인한 상태는 아래와 같다.

- 현재 브랜치: `codex/detailcheckout_260402`
- 원격 추적 상태: `origin/codex/detailcheckout_260402` 대비 `ahead 5`
- 현재 dirty 파일:
  - `PLAN.md`
  - `src/app/globals.css`
  ...
  - `docs/2026-04-07-multi-user-transition-plan.md`
  - `docs/2026-04-07-spreadsheet-grid-performance-plan.md`
  - `docs/worklogs/2026-04-07-daily-grid-performance-architecture-research.md`
  - `docs/worklogs/2026-04-07-daily-row-resize-fast-path.md`
  - `docs/worklogs/2026-04-07-multi-user-transition-plan.md`
  - `docs/worklogs/2026-04-07-spreadsheet-grid-performance-plan-docs.md`

중요:

- 위 목록은 문서 작성 시점 기준이다.
- 실제 실행 직전에는 반드시 다시 `git status -sb`로 상태를 재확인한다.
...
- `codex/detailcheckout_260402`
  - 현재 작업의 원래 브랜치
  - 이후 최종 통합을 받는 대상 브랜치
- `codex/grid-baseline-20260407`
  - 현재 시점의 전체 변경 상태를 고정하는 체크포인트 브랜치
- `codex/grid-integration-20260407`
...
1. 현재 dirty 상태는 `stash`가 아니라 `baseline commit`으로 고정한다.
...
```powershell
git switch -c codex/grid-baseline-20260407
```

설명:

- 현재 dirty 워크트리를 그대로 새 브랜치로 옮긴다.
- 아직 커밋은 하지 않는다.
...
```powershell
git add -A
git commit -m "chore: checkpoint spreadsheet grid baseline 2026-04-07"
```
...
```powershell
git switch -c codex/grid-integration-20260407
```
...
```powershell
git switch codex/grid-integration-20260407
git switch -c codex/grid-p1-virtualization
```
...
```powershell
git branch --merged codex/grid-integration-20260407
```
...
git merge --no-ff codex/grid-integration-20260407
```
