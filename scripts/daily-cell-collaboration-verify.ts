...
const updateVersionMigrationSql = read("prisma/migrations/202606080002_add_task_cell_update_versions/migration.sql");
...
assert.match(prismaSchema, /documentVersion\s+Int\s+@default\(1\) @map\("document_version"\)/);
assert.match(prismaSchema, /@@unique\(\[projectId, taskId, fieldKey\]\)/);
assert.match(prismaSchema, /@@index\(\[cellDocumentId, documentVersion\]\)/);
...
assert.match(updateVersionMigrationSql, /document_version integer not null default 1/);
assert.match(updateVersionMigrationSql, /task_cell_updates_document_version/);
...
assert.match(snapshotRouteSource, /knownVersion/);
assert.match(snapshotRouteSource, /stateVector/);
...
assert.match(serviceSource, /MAX_RETAINED_CELL_UPDATES/);
assert.match(serviceSource, /documentVersion/);
assert.match(serviceSource, /listCatchUpUpdates/);
assert.match(serviceSource, /buildYStateDiffBase64/);
assert.match(serviceSource, /pruneTaskCellDocumentUpdates/);
assert.match(serviceSource, /delete from task_cell_updates/);
...
assert.match(journalSource, /listCellDocumentJournalOperations/);
assert.match(journalSource, /updateCellDocumentJournalOperation/);
...
assert.match(hookSource, /drainOutbox/);
assert.match(hookSource, /listCellDocumentJournalOperations/);
assert.match(hookSource, /knownVersion/);
assert.match(hookSource, /pendingUpdatesRef\.current\.length > 0/);
assert.match(hookSource, /publishCellDocumentUpdateEvent/);
assert.doesNotMatch(hookSource, /postgres_changes/);
assert.doesNotMatch(hookSource, /task_cell_documents/);
