...
async function main() {
  ...
  const { defaultProjectName, localFileStorePath, localProjectMetaPath, localTaskStorePath, localUploadRoot } = await import("../src/lib/runtime-config");
  ...
  for (const task of [...normalizedTasks].sort((left, right) => left.depth - right.depth || left.taskNumber - right.taskNumber)) {
    ...
    await prisma.task.upsert({
      ...
      update: {
        ...
        progressNote: normalizeText(task.legacy.progressNote),
        conclusion: normalizeText(task.legacy.conclusion),
        fileMemo: normalizeText(task.legacy.fileMemo),
